import type { ProcessedEmployee, TenureStatistics } from '@/types';
...
export class StatisticsEngine {
  ...
  private generateHistogram(tenures: number[]): TenureStatistics['histogram'] {
    const bins: TenureStatistics['histogram'] = {
      '0-6m': 0,
      '6-12m': 0,
      '1-2y': 0,
      '2-3y': 0,
  ...
  private assessQuality(employees: ProcessedEmployee[]): TenureStatistics['dataQuality'] {
    return {
  ...
  private getEmptyStats(): TenureStatistics {
    return {
      count: 0,
      currentCount: 0,
      pastCount: 0,
