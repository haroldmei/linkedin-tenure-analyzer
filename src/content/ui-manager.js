export class UIManager {
    ...
    showProgress(current = 0, total = 0) {
        this.hideResults();
    ...
    updateProgress(current, total) {
        if (!this.progressElement)
            return;
        const progressBar = this.progressElement.querySelector('.tenure-analyzer-progress-fill');
        const progressText = this.progressElement.querySelector('.tenure-analyzer-progress-text');
    ...
    showResults(stats) {
        ...
        this.announceStatus(`Analysis complete. Median tenure is ${stats.median} months.`, 'assertive');
    }
    hideResults() {
        if (this.resultsElement) {
        ...
        ${bins
            ...
            return `
              <div style="display: flex; align-items: center; gap: 12px;">
                <div style="min-width: 60px; font-size: 12px; color: #666;">${this.escapeHtml(label)}</div>
